...
class VolcVideoParams(BaseModel):
    ...
    ratio: Optional[str] = Field(None, description="Aspect ratio (not supported by all models, e.g. t2v models)")
    duration: int = Field(5, description="Duration in seconds")
    generate_audio: bool = Field(False, description="Generate audio (only supported by seedance-1-5-pro model)")
